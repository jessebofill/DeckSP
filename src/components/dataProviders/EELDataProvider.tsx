...
export const EELDataProvider: FC17<{}> = ({ children }) => {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (settings) {
                setReady(false);
                const res = await Backend.getEELParams(settings.liveprog_file).catch(e => e instanceof Error ? e : new Error('Caught exception in EEL data handler'));
